    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 mt-4 mb-4">
        ...
        <div className="card">
          ...
          <p className="text-xl font-bold text-blue-600 whitespace-nowrap">Rs. {order.total?.toLocaleString()}</p>
        </div>
      </div>
      ...
      {order.fraud_result && (
        <div className="card mb-4">
          ...
          <p className="text-xs text-gray-500 bg-gray-50 rounded p-2 mb-2">
            Formula: (Rule x 0.40) + (Identity x 0.20) + (ML x 0.40)
          </p>
      ...
      <div className="card mb-4">
        ...
        {order.items?.map((item: any, i: number) => (
          <div key={i} className="flex justify-between py-2 border-b last:border-0 text-sm">
            ...
            <span className="font-medium whitespace-nowrap">Rs. {item.line_total?.toLocaleString()}</span>
          </div>
        ))}
      </div>
      ...
      <div className="card bg-blue-50 border-blue-200">
        <h2 className="font-semibold mb-2 text-blue-800 text-sm">Security Details</h2>
        <div className="text-xs text-blue-700 space-y-1">
          ...
          <p className="break-all">SHA-256 Hash: <code className="bg-blue-100 px-1 rounded">{order.integrity_hash}</code></p>
        </div>
      </div>
    </div>
  )
