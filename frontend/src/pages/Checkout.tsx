...
export default function Checkout() {
  ...
  useEffect(() => {
    api.get('/profile').then(r => {
      if (r.data.address?.full_name) { setAddr(r.data.address); setHasSaved(true) }
    }).catch(() => {})
    api.get('/profile/card-raw').then(r => {
      if (r.data.card?.card_number) {
        const c = r.data.card
        setCard({ card_number: c.card_number, cvv: '', expiry: c.expiry || '', card_brand: c.card_brand || 'Visa' })
        setCardDisplay(formatCardNumber(c.card_number))
        setHasSaved(true)
      }
    }).catch(() => {})
  }, [])
  ...
  function handleCardNumber(val: string) {
    const digits = val.replace(/\D/g, '').slice(0, 16)
    setCardDisplay(formatCardNumber(digits))
    setCard(c => ({ ...c, card_number: digits, card_brand: detectBrand(digits) }))
  }
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!addr.address_line1.trim()) { setError('Address Line 1 is required'); return }
    if (!addr.city.trim()) { setError('City is required'); return }
    if (!addr.pincode.trim()) { setError('Pincode is required'); return }
    if (!card.card_number.trim()) { setError('Card number is required'); return }
    if (!card.expiry.trim()) { setError('Card expiry is required'); return }
    if (!card.cvv.trim()) { setError('CVV is required'); return }
    ...
    try {
      ...
      setError(err.response?.data?.detail || err.message || 'Order failed. Please try again.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="card text-center">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm mb-4">
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <p className="font-bold text-blue-600 whitespace-nowrap">₹{result.total?.toLocaleString()}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <p className={`font-bold capitalize ${result.fraud_result?.risk_level === 'low' ? 'text-green-600' : result.fraud_result?.risk_level === 'medium' ? 'text-yellow-600' : 'text-red-600'}`}>
              {result.fraud_result?.risk_level}
            </p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
        ...
        {result.fraud_result?.flags?.length > 0 && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-left mb-4">
            ...
            {result.fraud_result.flags.map((f: string, i: number) => <p key={i} className="text-yellow-700 text-xs">• {f}</p>)}
          </div>
        )}
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-left mb-4">
          ...
          <p className="text-blue-700 text-xs break-all">• SHA-256: {result.integrity_hash?.slice(0, 40)}...</p>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="card mb-4">
        ...
        {cart.map(i => (
          <div key={i.product_id} className="flex items-center justify-between text-sm py-2 gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <img src={i.image} alt={i.name} className="w-10 h-10 object-cover rounded bg-gray-100 flex-shrink-0" />
              <span className="truncate">{i.name} × {i.qty}</span>
            </div>
            <span className="font-medium whitespace-nowrap flex-shrink-0">₹{(i.price * i.qty).toLocaleString()}</span>
          </div>
        ))}
        <div className="border-t mt-2 pt-2 flex justify-between font-bold">
          <span>Total</span>
          <span className="text-blue-600 whitespace-nowrap">₹{total.toLocaleString()}</span>
        </div>
      </div>

      {error && <div className="bg-red-50 border border-red-200 text-red-700 rounded-lg p-3 mb-4 text-sm">{error}</div>}
      {hasSaved && <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4 text-sm text-green-700">✓ Pre-filled from your saved profile.</div>}
      ...
      <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                {card.card_brand && card.card_number && (
                  <span className="ml-2 text-xs font-bold text-blue-600">{card.card_brand}</span>
                )}
              </label>
              <input className="input font-mono" placeholder="4111 1111 1111 1111"
                autoComplete="cc-number" aria-label="Card number"
                value={cardDisplay}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input className="input" placeholder="MM/YY" maxLength={5}
                  autoComplete="cc-exp" aria-label="Card expiry"
                  value={card.expiry} onChange={e => handleExpiry(e.target.value)} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">CVV</label>
                <input className="input" placeholder="123" maxLength={4} type="password"
                  autoComplete="cc-csc" aria-label="CVV"
        ...
        {/* Save details — unchecked by default */}
        ...
        <button type="submit" className="btn-primary w-full py-3 text-base" disabled={loading || cart.length === 0}>
          {loading
            ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>Processing securely...</span>
